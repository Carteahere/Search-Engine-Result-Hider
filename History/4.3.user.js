...
(function() {
    ...
    function showConfigPanel() {
        ...
        panel.innerHTML = `
            ...
            <div style="margin-bottom: 8px;">
                ...
                example.* 匹配域名 | title/.*文本.* 匹配标题 | text/.*文本.* 匹配内容<br>
                title/(?i).*AbC.* 加(?i)忽略大小写 | title/.*A(B|C).* 同时匹配AB和AC
                </div>
            </div>
    ...
    function testRules() {
        ...
        if (ruleStatsArray.length > 0) {
            ...
            resultHTML = '<div style="color: #38a169; padding: 8px; border-radius: 4px; font-size: 12px; background: #f0fff4; text-align: center;">无匹配项</div>';
        }
