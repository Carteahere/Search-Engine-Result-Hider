...
3. 地址只支持**https**和完整路径，如坚果云`https://dav.jianguoyun.com/dav/your_folder/`，路径文件夹时不存在会自动创建，文件名如`rules.txt`修改后需手动上传覆盖一次
4. 油猴无安全存储API，密码只能本地混淆处理，安全起见必须使用单独应用密码
4. 同步时为确保时间戳准确，会自动访问一次阿里/cloudflare/worldtimeapi授时点，若有跨域请求权限申请弹窗选`总是允许`，拒绝则默认使用远程webdav的date时间戳
