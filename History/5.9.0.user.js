...
(function() {
	...
	function updateBubbleContent(statusBtn, blocked) {
		...
		if (currentConfig.showCount) {
			statusBtn.textContent = isLeft ? `🚫 ${blocked}` : `${blocked} 🚫`;
		} else {
	...
	function updateStatus(blocked) {
		...
		if (!status) {
			...
			function endDrag(e) {
				...
				status.style.transition = 'opacity 0.2s, text-shadow 0.2s, transform 0.2s, left 0.3s ease, right 0.3s ease, top 0.3s ease';
    ...
    `;
		...
		setTimeout(() => {
			const closeHandler = (e) => {
				if (e.target.closest('#searchfilter-status')) return;
	...
	function registerMenu() {
		...
		const panelCenteredLabel = currentConfig.panelCentered ? "🟢 面板居中：启用" : "🔴 面板居中：关闭";
		GM_registerMenuCommand(panelCenteredLabel, () => {
