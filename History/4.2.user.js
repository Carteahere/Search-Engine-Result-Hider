...
(function() {
    ...
    function getResultSnippet(result, engine) {
        ...
        const googleSnippetSelectors = ['.VwiC3b', '.st', '.s3v9rd', '.IsZvec', '.lyLwlc', '.yXK7lf'];
        const duckSnippetSelectors = ['.result__snippet', '.tile__snippet', '.snippet', '.result__snippet.js-result-snippet', '.js-result-snippet'];
    ...
    function blockResults() {
        ...
        results.forEach(result => {
            ...
            const snippet = getResultSnippet(result, engine);  // 新增
    ...
    function testRules() {
        ...
        const maxHeight = Math.max(rulesTextarea.offsetHeight * 1.5, 180);
        ...
        if (ruleStatsArray.length > 0) {
            ...
            resultHTML = '<div style="color: #38a169; padding: 8px; border-radius: 4px; font-size: 12px; background: #f0fff4;">无匹配项</div>';
        }
