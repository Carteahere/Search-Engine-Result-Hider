...
优先级：高亮 > 白名单，但黑名单 > 高亮  
注意：`@N`和非`*://`开头规则之间空格隔开；只支持5种颜色，即`@N`为`@1`～`@5`，通过脚本菜单打开自定义颜色面板

### 2.7 复合规则：

**说明：**
1. 在规则后添加 `@if(...)` 作为附加条件，规则和 `@if` 之间必须空格隔开，多个 `@if` 条件同时生效(逻辑与`&`，可合并为单个`@if`)，复合规则默认忽略大小写
...
| `titles` | string \| string\[\] | 可选，标题选择器列表，一键屏蔽按钮依赖标题选择器定位，置空则无按钮 |
